# Third-Party Licenses

This repository contains code from other projects. Files containing such code include a license notice, and copies of the applicable licenses are stored in this folder.

## fuzzysort (MIT)

- **File**: `modules/common/functions/fuzzysort.js`
- **Source**: https://github.com/farzher/fuzzysort
- **License**: [MIT](MIT.txt)
- **Copyright**: (c) 2018 Stephen Kamenar

## thumbnail-generator-ubuntu (MIT)

- **File**: `scripts/thumbnails/thumbgen.py`
- **Source**: https://github.com/difference-engine/thumbnail-generator-ubuntu
- **License**: [MIT](MIT.txt)
- **Copyright**: (c) difference-engine contributors
