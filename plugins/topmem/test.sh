#!/bin/bash
# Top Memory plugin tests

export HAMR_TEST_MODE=1
source "$(dirname "$0")/../test-helpers.sh"

TEST_NAME="Top Memory Plugin Tests"
...
test_initial_has_mock_processes() {
    local result=$(hamr_test initial)
    # Mock data has 3 processes (sorted by memory: code > firefox > python3)
...
test_search_filters_by_name() {
    local result=$(hamr_test search --query "code")
    assert_type "$result" "results"
    assert_has_result "$result" "proc:5678"
    assert_no_result "$result" "proc:1234"  # firefox
    assert_no_result "$result" "proc:9012"  # python3
}

test_search_filters_by_pid() {
    local result=$(hamr_test search --query "9012")
    assert_type "$result" "results"
    assert_has_result "$result" "proc:9012"
...
test_poll_with_query() {
    local result=$(hamr_test poll --query "code")
    assert_type "$result" "results"
    assert_has_result "$result" "proc:5678"
...
test_action_kill() {
    local result=$(hamr_test action --id "proc:5678" --action "kill")
    assert_type "$result" "results"
...
test_action_kill9() {
    local result=$(hamr_test action --id "proc:5678" --action "kill9")
    assert_type "$result" "results"
    assert_contains "$result" "killed"
}
...
test_action_default_is_kill() {
    ...
    local result=$(hamr_test action --id "proc:5678")
    assert_type "$result" "results"
    assert_contains "$result" "killed"
}
